...
describe("getPendingActions", () => {
  ...
  it("schweigt, sobald ein eigenes Logbuch existiert", async () => {
    ...
    await sql`INSERT INTO mission_participants (mission_id, character_id) VALUES (${mission.id}, ${figur.id})`;
    await sql`
      INSERT INTO mission_logs (slug, mission_id, title, author_id, owner_user_id, content, metadata)
      VALUES ('mein-log', ${mission.id}, 'Mein Log', ${figur.id}, ${user.id}, '<p>x</p>', '{}')
    `;
  ...
  it("meldet ein Gespräch, in dem ich am Zug bin", async () => {
    ...
    const [entry] = await sql<{ id: number }[]>`
      ...
      VALUES ('plausch', 'Plausch', 'dialogue', '', '{}', '{}', NULL, TRUE, ${ich.id})
      RETURNING id
    `;
  ...
  it("übergeht ein Gespräch, an dem ich gar nicht beteiligt bin", async () => {
    ...
    const [entry] = await sql<{ id: number }[]>`
      INSERT INTO archive_entries (slug, title, category, content, tags, metadata, source_md, dialogue_open, owner_user_id)
      VALUES ('fremd', 'Fremdes Gespräch', 'dialogue', '', '{}', '{}', NULL, TRUE, ${andere.id})
      RETURNING id
    `;

...
Do local tests (unit, e2e, no integration) with each commit. Also do a full scope code review each time you are instrcuted to merge. Fix all problems found while reviewing and repeat unless no more problems occur. Only then and if Netlify shows green merge the PR.
